...
/**
 * Decrease the inventory levels for items from an order and update the amount of an item sold.
 *
...
function DecreaseInventoryFromOrder($orderId)
{
	...
	$query = sprintf("
		SELECT op.*, p.prodinvtrack, p.prodcurrentinv, p.prodlowinv, v.vcstock, v.vclowstock, GROUP_CONCAT(a.assocprodid) AS assocprodids
		FROM [|PREFIX|]order_products op
		LEFT JOIN [|PREFIX|]products p ON (p.productid=op.ordprodid)
		LEFT JOIN [|PREFIX|]product_associations a ON (a.assocbaseid=op.ordprodid)
	...
	$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
	while ($product = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
		// Explode the comma delimited list of product associations
		$product['assocprodids'] = explode(',', $product['assocprodids']);
		
		// Actually adjust the inventory
		AdjustProductInventory($product['ordprodid'], $product['ordprodvariationid'], $product['prodinvtrack'], '-'.$product['ordprodqty'], $product['assocprodids'], $orderId);
 ...
 */
function AdjustProductInventory($productId, $variationId, $inventoryTracking, $inventoryAdjustment, $associationIds = false, $orderId = 0)
{
	$queries = array();
	...
	if(gzte11(ISC_MEDIUMPRINT)) {
		...
		// Adjust inventory for all of the associated products
		if($associationIds) {
			foreach($associationIds as $associationId) {
				$query = sprintf("
					SELECT op.*, p.prodinvtrack, p.prodcurrentinv, p.prodlowinv, vv.combinationid, GROUP_CONCAT(a.assocprodid) AS assocprodids
					FROM [|PREFIX|]products p
					LEFT JOIN [|PREFIX|]product_associations a ON (a.assocbaseid=p.productid)
					LEFT JOIN [|PREFIX|]order_products op ON (orderorderid='%d')
					LEFT JOIN [|PREFIX|]product_variation_combinations v ON ( v.combinationid=op.ordprodvariationid )
					LEFT JOIN [|PREFIX|]product_variation_combinations vv ON ( 
						vv.vcvariationid=v.vcvariationid and
						vv.vcoptionids=v.vcoptionids and
						vv.vcproductid='%d' 
					)
					WHERE p.productid='%d' and ordprodtype!=3",
					$orderId,
					$associationId,
					$associationId
				);
				$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
				while ($product = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) { 
					AdjustProductInventory($associationId, $product['combinationid'], $product['prodinvtrack'], '-'.$product['ordprodqty'], $product['assocprodids'], $orderId);
				}
			}
		}
...
function UpdateOrderStatus($orderIds, $status, $email=true, $preventModuleUpdateCallback=false)
{
	...
	foreach($orderIds as $orderId) {
		...
		// If the order has a referral code associated, give them credit now
		if(OrderIsComplete($status)) {
			
			$refquery = sprintf("SELECT ordreferralid FROM [|PREFIX|]order_referrals WHERE ordreforderid='%d'", $GLOBALS['ISC_CLASS_DB']->Quote($orderId));
			$refresult = $GLOBALS['ISC_CLASS_DB']->Query($refquery);
			if ($GLOBALS['ISC_CLASS_DB']->CountResult($refresult) > 0) {
				$refquery = sprintf("SELECT * FROM [|PREFIX|]orders WHERE orderid='%d'", $GLOBALS['ISC_CLASS_DB']->Quote($orderId));
				$refresult = $GLOBALS['ISC_CLASS_DB']->Query($refquery);
				$order = $GLOBALS['ISC_CLASS_DB']->Fetch($refresult);
				
				// If we have a referral code applied to this order and the order is over $40, we now need to increment the referring user's credit
				if ((int)$order['subtotal_ex_tax'] >= 40) {
					$query = "
						SELECT * FROM [|PREFIX|]order_referrals t1, [|PREFIX|]refcodes t2
						WHERE t1.ordreferralid = t2.refcodeid
						AND t1.ordreforderid='".$order['orderid']."'
					";
					$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
					$ref = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
		
					if(!empty($ref)) {
						$query = sprintf("SELECT customerid, custstorecredit FROM [|PREFIX|]customers WHERE customerid='%d'", $ref['refcodeuserid']);
						$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
						...
						if($continueRef == true) {
							$newCredit = $customer['custstorecredit'] + $ref['refreward'];
							$updatedCustomer = array(
								"custstorecredit" => DefaultPriceFormat($newCredit)
							);
							$updatedStatus = array(
								"ordrefstatus" => 1
							);	
							$GLOBALS['ISC_CLASS_DB']->UpdateQuery("customers", $updatedCustomer, "customerid='".$GLOBALS['ISC_CLASS_DB']->Quote($customer['customerid'])."'");
							$GLOBALS['ISC_CLASS_DB']->UpdateQuery("order_referrals", $updatedStatus, "ordreforderid='".$GLOBALS['ISC_CLASS_DB']->Quote($order['orderid'])."'");
				
							// Log the credit change
							$creditChange = CFloat($ref['refreward']);
							if($creditChange != 0) {
								$creditLog = array(
