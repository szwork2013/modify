...
class ISC_ENTITY_ORDER extends ISC_ENTITY_BASE
{
	...
	protected function parseInput($input)
	{
		...
		$order = array(
			...
			'ref_discount'				    => $quote->getReferralDiscount(),
	...
	private function commitReferrals($orderId, $rawInput, $editingExisting = false)
	{
		// Delete existing referral codes for this order
		if($editingExisting) {
			$GLOBALS['ISC_CLASS_DB']->deleteQuery('order_referrals',
				"WHERE ordreforderid='".(int)$orderId."'"
			);
			
		}
		
		$quote = $rawInput['quote'];
		$ref = $quote->getAppliedRefCodes();
		
		if(!empty($ref['id'])) {
			$newReferral = array(
				'ordreforderid' => $orderId,
				'ordreferralid' => $ref['id'],
				'ordrefcode' => $ref['code'],
				'ordrefamount' => $ref['discountAmount'],
				'applied_discount' => $ref['totalDiscount']
			);
			$refInsertQuery = $GLOBALS['ISC_CLASS_DB']->insertQuery('order_referrals', $newReferral);
			if(!$refInsertQuery) {
				return false;
			}
		}
	...
	private function commitProducts($orderId, $rawInput, $itemAddressMap, $editingExisting=false, $adjustInventory=true)
	{
			...
			if ($adjustInventory && $existingOrder && $existingOrder['ordinventoryupdated']) {
				...
				if ($existingProduct && $existingProduct['ordprodid']) {
					...
					if ($inventoryRequiresAdjustment) {
						...
						// put old product inventory back to store
							$query = sprintf("
								SELECT GROUP_CONCAT(a.assocprodid) AS assocprodids
								FROM [|PREFIX|]product_associations
								WHERE assocbaseid='%d'",
								$existingProduct['ordprodid']
							);
							...
							while ($product = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
								...
								if (!AdjustProductInventory($existingProduct['ordprodid'], $existingProduct['ordprodvariationid'], $product['prodinvtrack'], '+' . $existingProduct['ordprodqty'], $product['assocprodids'], $orderId)) {
				...
				if ($inventoryRequiresAdjustment && $item->getProductId()) {
					...
					while ($product = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
						// Explode the comma delimited list of product associations
						$product['assocprodids'] = explode(',', $product['assocprodids']);
						if (!AdjustProductInventory($item->getProductId(), $item->getVariationId(), $item->getInventoryTrackingMethod(), '-' . $item->getQuantity(), $product['assocprodids'], $orderId)) {
		...
		if(!empty($giftCertificates)) {
			getClass('ISC_GIFTCERTIFICATES')->GiftCertificatesFromOrder(
				$orderId,
				$giftCertificates,
				1
			);
	...
	*/
	public function deleteOrderProduct($orderProductId, $adjustInventory = true)
	{
		...
		$orderProduct['assocprodids'] = explode(',', $orderProduct['assocprodids']);
		if (!AdjustProductInventory($orderProduct['ordprodid'], $orderProduct['ordprodvariationid'], $orderProduct['prodinvtrack'], '+' . $orderProduct['ordprodqty'], $orderProduct['assocprodids'], $orderId)) {
	...
	*/
	...
	protected function addPosthook($orderId, $savedata, $rawInput)
	{
		...
		if(!$this->commitReferrals($orderId, $rawInput, false)) {
			return false;
		}
	...
	protected function editPosthook($orderId, $savedata, $rawInput)
	{
		...
		if (!$this->commitReferrals($orderId, $rawInput, true)) {
			return false;
		}
	...
	protected function deletePosthook($orderId, $order, $deleteGiftCertificates=false)
	{
		...
		$queries = array(
			...
			"DELETE FROM [|PREFIX|]order_referra;s WHERE ordreforderid = " . (int)$orderId,
	...
	public function convertOrderToQuote($orderId, $enableDiscounts = true)
	{
		...
		// Add any applied referral codes
		$query = "
			SELECT *
			FROM [|PREFIX|]order_referrals
			WHERE ordreforderid='".$order['orderid']."'
		";
		$result = $GLOBALS['ISC_CLASS_DB']->query($query);
		while($ref = $GLOBALS['ISC_CLASS_DB']->fetch($result)) {
			$quoteReferral = array(
				'id' => 0,
				'code' => $ref['ordrefcode'],
				'discountAmount' => $ref['ordrefonamount'],
				'totalDiscount' => $ref['applied_discount'],
			);
			$quote->addRefCode($quoteReferral);
		}
