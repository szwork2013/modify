...
class ISC_ENTITY_PRODUCT extends ISC_ENTITY_BASE
{
	...
	protected function addPrehook(&$savedata, $rawInput)
	{
		...
		if (!array_key_exists("prodassociation", $savedata)) {
			$savedata["prodassociation"] = 0;
		}
		
		if (!array_key_exists("prodassociationtype", $savedata)) {
			$savedata["prodassociationtype"] = NULL;
		}
