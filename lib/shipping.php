	...
	function GetShippingZoneIdByAddress($address)
	{
		...
		if($address['shipzip']) {
			$query = "
				...
				WHERE z.zoneenabled='1' AND locationtype='zip' AND locationcountryid='".(int)$address['shipcountryid']."' AND '".$GLOBALS['ISC_CLASS_DB']->Quote($address['shipzip'])."' REGEXP REPLACE(REPLACE(CONCAT('^', locationvalue, '$'), '*', '.{1,}'), '?',  '.')
			";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
