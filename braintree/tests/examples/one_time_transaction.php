...
<html>
    <head>
        <title>Braintree Transparent Redirect Example</title>
    </head>
    <body>
        <?php
        if (isset($_GET["id"])) {
        ...
        <?php
            ...
            <form method="POST" action="<?php echo Braintree_TransparentRedirect::url() ?>" autocomplete="off">
                ...
                <?php $tr_data = Braintree_TransparentRedirect::transactionData(
                    array('redirectUrl' => "https://" . $_SERVER["SERVER_NAME"] . ":" . $_SERVER["SERVER_PORT"] . parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH),
