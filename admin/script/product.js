...
function CheckAddProductForm() {
	...
	if ($("#ProductType_0").attr('checked')) {
		if(!document.getElementById('physProdTypeFace').checked && !document.getElementById('physProdTypeStrap').checked && !document.getElementById('physProdTypeMod').checked && !document.getElementById('physProdTypeSet').checked && !document.getElementById('physProdTypeOther').checked) {
			alert(lang.PleaseChooseAProductType);
			return false;
		}
	}
	...
	if(document.getElementById('useProdAssociationYes').checked) {
	  if(!document.getElementById('prodAssociationTypePseudo').checked && !document.getElementById('prodAssociationTypeInventory').checked) {
	  	alert(lang.ProductAssociationError);
  	    return false;
	  }
	  if ($("#ProductSelect option").length == 0) {
		alert(lang.ProductAssociationEmptyError);
		return false;
	  }
	}
...
$('#useProdAssociationYes').click(function() {
	$('#associationList').show();
});

$('#useProdAssociationNo').click(function() {
...
$(function() {
	...
	if($("#physProdTypeMod").is(':checked')) {
		$(".BodyContainer tbody tr:nth-child(4) :input").attr("disabled", true);
		$("#prodName, #prodCode").attr("disabled", false);
	}
