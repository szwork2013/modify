	...
	var options = {
		...
		success: function (xml) {
			...
			$('component', xml).each(function(){
				...
				// image reported as new component by server -- set it but don't update it
				image.setComponent(true, false);
			});
...
ProductImages.refreshSortable = function () {
	...
	for (var i = ProductImages.images.length; i--;) {
		ProductImages.images[i].setComponent(ProductImages.images[i].getComponent());
	}
};

// javascript data model used for managing product images
ProductImages.Image = function (id, productId, preview, zoom, original, description, baseThumbnail, sort, hash, component) {
	if (typeof id == 'object') {
	...
	var _id, _productId, _preview, _zoom, _original, _description, _baseThumbnail, _sort, _hash, _row, _component;
	...
	self.setComponent = function (component, update) {
		if (typeof update == 'undefined') {
			update = false;
		}
		...
		if (update && _component !== component) {
			_ajax('updateComponent', { image: self.getId() });
		}

		_component = component;

		// if necessary, update radio elements in the dom - browser should do this automatically for same named radio buttons though
		var input = $('.productImageComponent input', self.getRow());
		if (input.attr('checked') !== _component) {
			input.attr('checked', _component);
		}
	};
	...
	self.onComponentClick = function (evt) {
		// should always be true but, just in case...
		var checked = $(evt.target).attr('checked');

		self.setComponent(checked, true);

		// set all other images as the opposite - server side code will handle updating all other db records
		for (var i = ProductImages.images.length; i--;) {
			if (ProductImages.images[i] !== self) {
				ProductImages.images[i].setComponent(!checked);
			}
		}
	};
	...
	$('.productImageComponent input', self.getRow()).click(self.onComponentClick);
...
$(function(){
	...
	$('#productImagesList').bind('sortstop', function (evt, ui){
		...
		image.setComponent(image.getComponent());
	});
});
