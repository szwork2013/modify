<?php
	...
	class ISC_ADMIN_REMOTE extends ISC_ADMIN_REMOTE_BASE
	{
		public function HandleToDo()
		{
			...
			switch  ($what) {
				case 'productimages':
					...
					break;
				case 'photowallimages':
					$adminPhotowallImage = new ISC_ADMIN_PHOTOWALL_IMAGE();
					$adminPhotowallImage->routeRemoteRequest($this);
					break;
				...
				case "disablestoremaintenance":
					...
					break;
				case "savephotowalltags":
					$this->SavePhotowallTags();
					break;
				case "deletephotowalltags":
					$this->DeletePhotowallTags();
					break;
				case "photowallitemsearch":
					$this->PhotowallItemSearch();
					break;
			}
		}
		...
		private function GetInventoryLevels()
		{
			...
			if(isset($_REQUEST['p']) && isset($_REQUEST['i']) && isset($_REQUEST['v']) && isset($_REQUEST['t'])) {
				...
				if ($invType == 1) {
					...
					$query = sprintf("select prodcurrentinv, prodphystype, prodlowinv from [|PREFIX|]products where productid='%d'", $GLOBALS['ISC_CLASS_DB']->Quote($prodId));
					$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
					
					$disabled = ($row['prodphystype'] == 3 ? 'disabled' : '');
					if($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
						...
						printf("<td><input $disabled type='text' size='3' value='%d' name='stock_level_%d' id='stock_level_%d' /></td>", $row['prodcurrentinv'], $prodId, $prodId);
						echo "</tr>";
						echo "<tr>";
						echo "<td>";
						printf("<td>%s:</td>", GetLang("LowStockLevel"));
						printf("<td><input $disabled type='text' size='3' value='%d' name='stock_level_notify_%d' id='stock_level_notify_%d' /></td>", $row['prodlowinv'], $prodId, $prodId);
						echo "</tr>";
						echo "</table>";
						printf("<input $disabled class='StockButton' type='button' value='%s' onclick='UpdateStockLevel(%d, 0)' style='margin-left:110px' />&nbsp; <img src='images/ajax-blank.gif' id='loading%d' />", GetLang("Save"), $prodId, $prodId);
					}
				} else {
					$optionIds = array();
					...
					$query = "SELECT prodphystype FROM [|PREFIX|]products WHERE productid='".$prodId."'";
					$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
					...
					foreach($combinations as $row) {
						...
						echo "<td valign='top'><img src='images/nodejoin.gif' style='padding-top:5px' /></td>";
						printf("<td>%s:</td>", GetLang("CurrentStock"));
						printf("<td><input $disabled type='text' size='3' value='%d' name='stock_level_%d_%d' id='stock_level_%d_%d' /></td>", $row['vcstock'], $prodId, $row['combinationid'], $prodId, $row['combinationid']);
						echo "</tr>";
						echo "<tr>";
						echo "<td>";
						printf("<td>%s:</td>", GetLang("LowStockLevel"));
						printf("<td><input $disabled type='text' size='3' value='%d' name='stock_level_%d_%d' id='stock_level_notify_%d_%d' /></td>", $row['vclowstock'], $prodId, $row['combinationid'], $prodId, $row['combinationid']);
						echo "</tr>";
						echo "</table>";
					}

					echo "</div>";
					printf("<input $disabled class='StockButton' type='button' value='%s' onclick='UpdateStockLevel(%d, 1)' style='margin-left:130px' />&nbsp; <img src='images/ajax-blank.gif' id='loading%d' />", GetLang('Save'), $prodId, $prodId);
				}
			}
		}
		...
		*/
		...
		private function SavePhotowallTags() {
			$productid = intval($_REQUEST['message']);
			$photoid = intval(substr($_REQUEST['photoID'], 5));
			
			$query = "SELECT imageid
			FROM [|PREFIX|]photowall
			WHERE imageid = '" . $GLOBALS['ISC_CLASS_DB']->Quote($photoid)."'";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			...
			WHERE productid = '" . $GLOBALS['ISC_CLASS_DB']->Quote($productid)."'";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			...
			echo $GLOBALS['ISC_CLASS_DB']->InsertQuery("photowall_tags", $newTag);
		}
		
		private function DeletePhotowallTags() {
			$id = intval($_REQUEST['id']);
					
			$query = "DELETE FROM [|PREFIX|]photowall_tags WHERE id = ".$id;
			$GLOBALS['ISC_CLASS_DB']->Query($query);
		}
		
		private function PhotowallItemSearch()
		{
			if(empty($_REQUEST['q'])) {
				exit;	
			}
