<?php
	...
	class ISC_ADMIN_PRODUCT extends ISC_ADMIN_BASE
	{
		...
		*/
		...
		public function _GetProductData($ProductId = 0, &$RefArray = array())
		{
			...
			if ($ProductId == 0) {
				...
				if($RefArray['prodtype'] == "1") {
					$RefArray['prodphystype'] = $_POST['physProdType'];
				} else {
					$RefArray['prodphystype'] = 0;	
				}
				
				if($_POST['useProdAssociation'] == "1") {
					if ($_POST['prodAssociationType'] == 0 || $_POST['prodAssociationType'] == 1) {
						$RefArray['prodassociation'] = 1;
						$RefArray['prodassociationtype'] = $_POST['prodAssociationType'];
					}
				} else {
					$RefArray['prodassociation'] = 0;
				}
				...
				$GLOBALS['prodsubtitle'] = $_POST['prodSubTitle'];
				$GLOBALS['prodquote'] = $_POST['prodQuote'];
				$GLOBALS['prodquoteauthor'] = $_POST['prodQuoteAuthor'];
		...
		*/
		...
		public function _CommitProduct($ProductId, &$Data, &$Variations, &$Associations, &$CustomFields, $DiscountRules=array(), &$Err = null, &$ProductFields=array(), $isImport=false)
		{
			...
			if(!isset($GLOBALS['DoTransaction'])) {
			...
			if ($ProductId == 0) {
				...
				if($Data['prodphystype'] !== "3") {
					$GLOBALS['NewProductId'] = $prodId;
				} else {
					$GLOBALS['TempNewProductId'] = $prodId;
				}
			...
			if(isset($Data['prodvariationid']) && $Data['prodvariationid'] != 0 && isset($Variations) && is_array($Variations) && $Data['prodtype'] == PT_PHYSICAL) {
				...
				foreach($Variations as $Variation) {
					...
					/**
					 * Add Mods Variation Data
					 */
					 if(isset($Data['prodphystype']) && $Data['prodphystype'] == 3 && $ProductId == 0) {
						$GLOBALS['ISC_CLASS_DB']->InsertQuery("product_variation_combinations", $updatedCombo);
					 } else {
			...
			if($Data['prodassociation'] == 1) {
				$this->SaveProductAssociations($prodId, $Associations);	
			} 
			...
			// Build Combination Image for Mods
			if(isset($Data['prodphystype']) && $Data['prodphystype'] == 3) {
				$combinationImage = ISC_PRODUCT_IMAGE::generateCombinationImages($prodId);
				$imageObject = new ISC_PRODUCT_IMAGE();
				$imageObject->setDateAdded(time());
				$imageObject->setIsComponent(0);
				$imageObject->setIsThumbnail(1);
				$imageObject->setProductId($prodId);
				$imageObject->setProductName($Data['prodname']);
				$imageObject->setVisible(1);
				$imageObject->setSourceFilePath($combinationImage);
				
				$imageObject->saveToDatabase();
			}
		...
		public function _GetComponentIDs($type = 0, $id = 0) {
			if(!$type) {
				return false;
			}
		...
		public function _GetComponentVariationData($faceId = false, $strapId = false) {
			if(!$faceId || !$strapId) {
				return false;
			}
			...
			// Loop through the variations added to the stack. If the optionid is already in,
			// check to see which one has lower inventory. Only keep the low inventory number.
			while($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
				foreach($componentVariations as $variationKey => $variation) {
						if($variation['vcoptionids'] == $row['vcoptionids']) {
							if($variation['vcstock'] < $row['vcstock']) { 
								continue;
							} else {
								unset($componentVariations[$variationKey]);
							}
						}
				}
				...
				$temp = array(
					"combinationid"=>0,
					"vcproductid"=>0,
					"vcvariationid"=>$row['vcvariationid'],
					"vcenabled"=>1,
					"vcoptionids"=>$row['vcoptionids'],
					"vcsku"=>"",
					"vcpricediff"=>"",
					"vcprice"=>"0.00",
					"vcweightdiff"=>"",
					...
					"vcimagethumb"=>"",
					"vcstock"=>$row['vcstock'],
					"vclowstock"=>$row['vclowstock']
				);
				array_push($componentVariations, $temp);
			}
			
			return $componentVariations;
		}
		
		public function _GetSkeletonMod() {
				// This is our data skeleton for each mod
			$modData = array(
				"productid"=>0,
				"prodhash"=>"",
				"prodname"=>"",
		...
		public function _GenerateMods($componentType, &$err, $update = false) {
			// Gather IDs for the double loop that is to ensue.
			if($update) {
				$prodId	= (int)$_POST['productId'];
			} else {
				$prodId = $GLOBALS['NewProductId'];
			}
			
			if($componentType == PPT_FACE) {
				$faceIDs = $this->_GetComponentIDs(PPT_FACE, $prodId);
				$strapIDs = $this->_GetComponentIDs(PPT_STRAP);
			} else if ($componentType == PPT_STRAP) {
				$faceIDs = $this->_GetComponentIDs(PPT_FACE);
				$strapIDs = $this->_GetComponentIDs(PPT_STRAP, $prodId);
			} else {
				// This should never happen.
				throw new Exception('Product must be face or strap');
			}
			
			
			// Pull in the strap DB info into a ref aray
			$straps = array();
			while($strap =  $GLOBALS['ISC_CLASS_DB']->Fetch($strapIDs)) { 
				array_push($straps, $strap);
			}
			
			// Loop through Faces
			while($face =  $GLOBALS['ISC_CLASS_DB']->Fetch($faceIDs)) {
				// Get product data for this face
				$faceData = array();
				$this->_GetProductData($face['productid'], $faceData);
				
				// Now for each face let's go through the straps
				foreach($straps as $strap) {
					
					// Get product data for this this strap
					$strapData = array();
					$this->_GetProductData($strap['productid'], $strapData);
					
					$modData['arrData'] = $this->_GetSkeletonMod();
					...
					$GLOBALS['StrapName'] = $strapData['prodname'];
					$modData['arrData']['proddesc'] = $this->template->render('Snippets/ModProductDescription.html');
					
					$modData['arrData']['prodprice'] = $faceData['prodprice'] + $strapData['prodprice'];
					$modData['arrData']['prodcostprice'] = $faceData['prodcostprice'] + $strapData['prodcostprice'];
					$modData['arrData']['prodretailprice'] = $faceData['prodretailprice'] + $strapData['prodretailprice'];
					$modData['arrData']['prodsaleprice'] = $faceData['prodsaleprice'] + $strapData['prodsaleprice'];
					
					$modData['arrData']['prodvariationid'] = $faceData['prodvariationid'];
					
					// Update all Variation variables
					$modData['arrVariations'] = $this->_GetComponentVariationData($faceData['productid'], $strapData['productid']);
					
					$modData['arrAssociations'] = array(
						array(
							"assocprodid" => array($faceData['productid'], $strapData['productid'])
						)
					);
					
					$modData['arrCustomFields'] = array();
					$modData['discount'] = array();
					$modData['err'] = "";
					$modData['arrProductFields'] = null;
					...
					if($update) {
						$query = "
							SELECT DISTINCT
								assocbaseid
							FROM
								[|PREFIX|]product_associations
							WHERE
								assocprodid = ".$strapData['productid']."
							OR
								assocprodid = ".$faceData['productid']."
							GROUP BY
								assocbaseid
							HAVING
								count(*)=2
						";
						$queryResult = $GLOBALS["ISC_CLASS_DB"]->Query($query);
						$result = $GLOBALS["ISC_CLASS_DB"]->Fetch($queryResult);
						$id = $result['assocbaseid'];
					}
					
					if (!$this->_CommitProduct($id, $modData['arrData'], $modData['arrVariations'], $modData['arrAssociations'], $modData['arrCustomFields'], $modData['discount'], $modData['err'], $modData['arrProductFields'])) {
						$err = $modData['err'];
						return false;
					}	
				}
		...
		*/
		...
		public function DoDeleteProducts($ids, $recursive = false)
		{
			if(!is_array($ids)) {
				$ids = array($ids);
			}
			...
			$queries[] = sprintf("delete from [|PREFIX|]product_associations where assocprodid IN ('%s')", $prodids);
			$queries[] = sprintf("delete from [|PREFIX|]product_associations where assocbaseid IN ('%s')", $prodids);
			...
			if(!$this->_DeleteAssociatedProducts($prodids)) {
				$GLOBALS["ISC_CLASS_DB"]->Query("rollback");
				return false;
			}
		...
		*/
		...
		/**
		* _DeleteAssociatedProducts
		* Deletes products that contain these products as children
		*
		* @param String $ProductIds The id(s) of the products to delete varations for in CSV, such as 105,106
		*/
		public function _DeleteAssociatedProducts($ProductIds)
		{
			// Delete the product combination images from the file system
			$query = "
				SELECT
				...
				FROM
					[|PREFIX|]product_associations
				WHERE
					assocprodid IN ('" . $ProductIds . "')";
			$result = $GLOBALS["ISC_CLASS_DB"]->Query($query);
			while($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
				if(!$this->DoDeleteProducts($row['assocbaseid'], true)) {
					return false;
				}
			}
		...
		*/
		...
		public function ManageProductsGrid(&$numProducts)
		{
			...
			$validSortFields = array('productid', 'prodphystype', 'prodcode', 'currentinv', 'prodname', 'prodcalculatedprice', 'prodvisible', $featuredColumn, '_calc_prodstatus');
		...
		public function ManageProducts($MsgDesc = "", $MsgStatus = "")
		{
			...
			if(!gzte11(ISC_LARGEPRINT)) {
				$GLOBALS[base64_decode("SGlkZUJ1bGtFeHBvcnRCdXR0b24=")] = false;
			}
		...
		*/
		public function setupProductImageGlobals($productImages)
		{
			...
			foreach ($productImages as /** @var ISC_PRODUCT_IMAGE*/$productImage) {
				...
				$component = 'false';
				if ($productImage->getIsComponent()) {
					$component = 'true';
				}
				...
				$GLOBALS['productImage_javascriptInitialiseCode'] .= sprintf(
					'new ProductImages.Image({id:%1$d,product:%8$d,preview:%2$s,zoom:%3$s,original:%10$s,description:%4$s,baseThumbnail:%5$s,sort:%7$d,hash:%9$s,component:%11$s});',
		...
		public function AddProductStep2()
		{
			...
			$this->_GetAssociationData(0, $arrAssociations);

			if($arrData['prodtype'] == PPT_FACE || $arrData['prodtype'] == PPT_STRAP) {
				$GLOBALS['DoTransaction'] = false;
				$GLOBALS["ISC_CLASS_DB"]->Query("start transaction");
			}
			// Commit the values to the database
			if ($this->_CommitProduct(0, $arrData, $arrVariations, $arrAssociations, $arrCustomFields, $discount, $err, $arrProductFields)) {
				// Log this action
				$GLOBALS['ISC_CLASS_LOG']->LogAdminAction($GLOBALS['NewProductId'], $arrData['prodname']);
				
				// Let's auto generate mods if we are adding a face or strap
				if($arrData['prodphystype'] == PPT_FACE || $arrData['prodphystype'] == PPT_STRAP) {
					$componentType = $arrData['prodphystype'];
					
					if(!$this->_GenerateMods($componentType, $err)) {
						// The product commit failed
						$GLOBALS["ISC_CLASS_DB"]->Query("rollback");
		...
		public function AddProductStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false)
		{
			...
			if($PreservePost == true) {
				...
				if(isset($arrData['prodsubtitle'])) {
					$GLOBALS['ProdSubTitle'] = $arrData['prodsubtitle'];
					$GLOBALS['ProdQuote'] = $arrData['prodquote'];
					$GLOBALS['ProdQuoteAuthor'] = $arrData['prodquoteauthor'];
				}
			...
			// By default we have no associations selected
			$GLOBALS['IsNoAssociation'] = 'checked="checked"';
			$GLOBALS['HideAssociationList'] = "none";
		...
		*/
		...
		/**
		* GetAssociationsAsOptions
		* Get a list of associations as <OPTION>tags
		*
		* @param Int $NumVariations A reference variable to pass back how many associations there are
		* @return String
		*/
		public function GetAssociationsAsOptions($productId)
		{
			if($productId) {
				$query = "
					SELECT isc_product_associations.*, isc_products.prodname
					FROM isc_product_associations, isc_products
					WHERE assocbaseid='". $productId . "'
					AND assocprodid = productid";
				$result = $GLOBALS["ISC_CLASS_DB"]->Query($query);
				$NumAssociations = $GLOBALS["ISC_CLASS_DB"]->CountResult($result);
				$options = "";
	
				while($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
					$options .= sprintf("<option value='%d'>%s</option>", $row['assocprodid'], isc_html_escape($row['prodname']));
				}
		...
		public function EditProductStep2()
		{
			...
			// Commit the values to the database
			if ($this->_CommitProduct($prodId, $arrData, $arrVariations, $arrAssociations, $arrCustomFields, $discount, $err, $arrProductFields)) {
				$successMessage = sprintf(GetLang('ProductUpdatedSuccessfully'), isc_html_escape($arrData['prodname']), ProdLink($arrData['prodname']));
				
				// Let's auto update mods if we are adding a face or strap
				if($arrData['prodphystype'] == PPT_FACE || $arrData['prodphystype'] == PPT_STRAP) {
					$componentType = $arrData['prodphystype'];

					if(!$this->_GenerateMods($componentType, $err, true)) {
						// The product commit failed
						$GLOBALS["ISC_CLASS_DB"]->Query("rollback");
						error_log("Sorry bruv, something went awry: " . $err);
						FlashMessage(sprintf(GetLang('ErrProductNotAdded'), $err), MSG_ERROR);
					} else {
						// The product was commited successfully, commit the transaction
						$GLOBALS["ISC_CLASS_DB"]->Query("commit");
		...
		public function EditProductStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false)
		{
			...
			if (ProductExists($prodId)) {
				...
				if($arrData['prodtype'] == 1) {
					$GLOBALS["IsPhysProd_" . $arrData['prodphystype']] = 'checked';
				}
				...
				foreach ($arrData['product_images'] as /** @var ISC_PRODUCT_IMAGE */$productImage) {
					...
					$GLOBALS['productImage_javascriptInitialiseCode'] .= sprintf(
						'new ProductImages.Image({id:%1$d,product:%8$d,preview:%2$s,zoom:%3$s,original:%9$s,description:%4$s,baseThumbnail:%5$s,sort:%7$d,component:%10$s});',
						/*1*/ $productImage->getProductImageId(),
						/*2*/ isc_json_encode($preview),
						/*3*/ isc_json_encode($zoom),
						/*4*/ isc_json_encode($productImage->getDescription()),
				...
				if($arrData['prodassociation'] == 1) {
					$GLOBALS['IsYesAssociation'] = 'checked="checked"';
					if($arrData['prodassociationtype'] == 1) {
						$GLOBALS['IsPseudoAssociation'] = 'checked="checked"';
					}
					else if($arrData['prodassociationtype'] == 0) {
						$GLOBALS['IsInventoryAssociation'] = 'checked="checked"';	
					}
				}
				else {
					$GLOBALS['IsNoAssociation'] = 'checked="checked"';
				...
				$GLOBALS['AssociationOptions'] = $this->GetAssociationsAsOptions($prodId);
		...
		* @return Void
		*/
		public function _GetAssociationData($ProductId = 0, &$RefArray = array())
		{
			if($ProductId == 0) {
				if(isset($_POST['prodids'])) {
					$tmp['assocprodid'] = explode(",", $_POST['prodids']);
					array_push($RefArray, $tmp);
				}
			}
		}
		...
		*/
		...
		public function CopyProductStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false, $OriginalProductID=0)
		{
			...
			if (ProductExists($prodId)) {
				...
				if($arrData['prodtype'] == 1) {
					$GLOBALS["IsPhysProd_" . $arrData['prodphystype']] = 'checked="checked"';
				}
		...
		*/
		...
		public function SaveProductAssociations($productId, $associations)
		{
			$associations = $associations[0]['assocprodid'];
			$query = "
				DELETE FROM [|PREFIX|]product_associations
				WHERE assocbaseid='".(int)$productId."'
			";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);

			foreach($associations as $association) {
				$GLOBALS['ISC_CLASS_DB']->Query("
					INSERT INTO [|PREFIX|]product_associations
					(assocbaseid, assocprodid)
					VALUES ('".$productId."','".$association."')"
				);
			}
