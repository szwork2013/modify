...
//      BETA Subscription Tags
	  ...
	  $xml_data->Push('parameterized-urls');
		$xml_data->Push('parameterized-url', array('url' => "https://shareasale.com/sale.cfm?transtype=sale&merchantID=45751"));
			$xml_data->Push('parameters');
				$xml_data->Element('url-parameter', '', array('name' => "tracking", 'type' => "order-id"));
				$xml_data->Element('url-parameter', '', array('name' => "amount", 'type' => "order-subtotal"));
...
//          $ShippingPackage = $ship->ShippingPackage;
    ...
    function CheckoutButtonCode($size="large", $variant=true, $loc="en_US",
      ...
      $data = "<div id=\"GoogleButtonDiv\" style=\"width: ".$width."px\">";
      if ($this->variant == "text") {
        $data .= "<div align=center><form method=\"POST\" action=\"". 
                $this->checkout_url . "\"" . ($this->googleAnalytics_id?
                " onsubmit=\"setUrchinInputCode();\"":"") . ">
                <input type=\"hidden\" name=\"checkout-flow-support.merchant-checkout-flow-support.parameterized-urls.parameterized-url-1.url\" value=\"https://shareasale.com/sale.cfm?transtype=sale&amp;merchantID=45751\"/> 
