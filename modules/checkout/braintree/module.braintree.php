<?php
	...
	class CHECKOUT_BRAINTREE extends ISC_GENERIC_CREDITCARD
	{
		...
		*/
		...
		protected function _ConstructPostData($postData, $subAmount = array())
		{
			require_once 'lib/Braintree.php';
			...
			if(!empty($subAmount)) {
				...
				$extraInfo = unserialize($orderDetails['extrainfo']);
				if($customerid == 0 && !array_key_exists('createAccount', $extraInfo)) {
					$customerid = time();
					$this->SetError(GetLang('MustRegisterAccount'));
					return "accountError";
				}
				$createCustomer = Braintree_Customer::create(array(
					'id' => $customerid,
				...
				if($createCustomer->message == "Customer ID has already been taken." || $createCustomer->success) {
					$createCard = Braintree_CreditCard::create(array(
					...
					if($createCard->success) {
						foreach($subAmount as $amount) {
							$result = Braintree_Subscription::create(array(
							  'paymentMethodToken' => $createCard->creditCard->token,
							  'planId' => 'Mod_of_the_Month_Monthly_Plan',
							  'price' => $amount,
							  'trialPeriod' => false,
							  'options' => array(
		...
		*/
		public function CheckSubscription()
		{
			...
			foreach($items as $item) {
				if($item->getProductId() && $item->getType() == 5) {
					$subscription[] = $item->getTotal();
				}
			}
		...
		public function ProcessPaymentForm($dataSource = array())
		{
			...
			$result = $this->_ConstructPostData($postData, $subscription);

			if (!$result->success) {
				$this->SetError(GetLang('CreditCardGatewayFail') . " Error:" . $result->message);
				return false;
			}
			$result = $this->_HandleResponse($result);
			
			if ($result && !empty($subscription)) {
		...
		public function DoVoid($orderId, $transactionId, &$message = '')
		{
			...
			if($result->success) {
				...
				$voidSuccess = sprintf(GetLang('VoidSuccessLog'), $orderId, "");
		...
		public function DelayedCapture($order, &$message = '', $amt=0)
		{
			...
			if($result->success) { // 1 is a success, 2 is declined and 3 is an error
				...
				$delayedCaptureSuccess = sprintf(GetLang('DelayedCaptureSuccessLog'), $orderId, "Success");
				...
				//set the message that's displayed in the front end
				$message = sprintf(GetLang('DelayedCaptureFailed'), "There was a problem", "");

				//log the transaction in store logs
				$delayedCaptureError = sprintf(GetLang('DelayedCaptureError'), $orderId, "");
				$GLOBALS['ISC_CLASS_LOG']->LogSystemError(array('payment', $this->GetName()), $delayedCaptureError, "");
				return false;
			}
		}
