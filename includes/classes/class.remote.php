	...
	class ISC_REMOTE extends ISC_XML_PARSER
	{
		...
		public function HandleToDo()
		{
			...
			switch  ($what) {
				...
				case 'disabledesignmode':
					...
					break;
				case "getphotowallltags": {
					$this->GetPhotowallTags();
					break;
				}
				case "getshopdata": {
					$this->GetShopData();
				...
				case "getsandboxquote": {
					$this->SandboxQuoteData();
					break;
				}
				case "removefromsandbox": {
					$this->RemoveFromSandbox();
					break;
				}
				case "getshoppage": {
		...
		private function GetExpressCheckoutPaymentForm()
		{
			...
			if(!is_array($pendingResult)) {
				$response = array(
					'status' => 0,
					'errorMessage' => getLang('ProblemCreatingOrder') . " " . $pendingResult,
			...
			if(!$pendingResult) {			
				$response = array('status' => 0, 'errorMessage' => "There was an error. Please try again! :) Error Details: " . isc_json_encode($pendingResult), "changeStep" => "Confirmation");
				echo isc_json_encode($response);
				exit;
			}
		...
		private function GetPhotowallTags() {
			$photoid = (int)substr($_REQUEST['photoID'], 5);
			$result = $GLOBALS['ISC_CLASS_DB']->Query("SELECT * FROM [|PREFIX|]photowall_tags WHERE photoid='". $photoid . "'");
			$i = 0;
			while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				$tags[$i]['id'] = $row['id'];
				$tags[$i]['x'] = $row['x'];
				$tags[$i]['y'] = $row['y'];
				$tags[$i]['height'] = $row['height'];
				$tags[$i]['width'] = $row['width'];
				$productClass = new ISC_PRODUCT($row['productid']);
				$tags[$i]['message'] = $productClass->GetProductName();
				$tags[$i]['url'] = ProdLink($tags[$i]['message']);
				$i++;
			}
			echo isc_json_encode($tags);
		}
		...
		*/
		...
		private function GetShopData() {			
			...
			$data = $GLOBALS['ISC_CLASS_REMOTE_CATEGORY']->GetJSON();
			die($data);
		}
		
		private function GetPagedProductData() {
			$GLOBALS['ISC_CLASS_REMOTE_CATEGORY'] = GetClass('ISC_REMOTE_CATEGORY');
			$data = $GLOBALS['ISC_CLASS_REMOTE_CATEGORY']->GetPagedProductData();
			die(json_encode($data));
		}
		
		private function SandboxQuoteData() {
			$GLOBALS['ISC_CLASS_SANDBOX'] = GetClass('ISC_SANDBOX');
			$data = $GLOBALS['ISC_CLASS_SANDBOX']->GetQuoteJSON();
			die($data);
		}
		
		private function GetCartData() {
			$GLOBALS['ISC_CLASS_CART'] = GetClass('ISC_CART');
			$data = $GLOBALS['ISC_CLASS_CART']->GetJSON();
			die($data);
		}
		
		private function GetSandboxData() {
			$GLOBALS['ISC_CLASS_SANDBOX'] = GetClass('ISC_SANDBOX');
			$data = $GLOBALS['ISC_CLASS_SANDBOX']->GetJSON();
			die($data);
		}
		
		private function RemoveFromSandbox() {
			$GLOBALS['ISC_CLASS_SANDBOX'] = GetClass('ISC_SANDBOX');
			$GLOBALS['ISC_CLASS_SANDBOX']->RemoteRemoveFromSandbox();
			$data = $GLOBALS['ISC_CLASS_SANDBOX']->GetJSON();
