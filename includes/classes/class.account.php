...
require_once(ISC_BASE_PATH . '/lib/addressvalidation.php');
	...
	class ISC_ACCOUNT
	{
		...
		*	Add a new shipping address to the customer's account
		*/
		static function AddShippingAddressRemote($MsgDesc = "", $MsgStatus = "")
		{
			$GLOBALS['HideAddShippingAddressMessage'] = 'none';
			if ($MsgDesc !== '') {
				$GLOBALS['HideAddShippingAddressMessage'] = '';
			...
			return $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ShippingAddressForm");
		}
		...
		*/
		public function GetCustomerReferrals(&$Result)
		{
			$query = "SELECT * FROM [|PREFIX|]orders o, [|PREFIX|]order_referrals r, [|PREFIX|]refcodes u 
						WHERE u.refcodeuserid = " . $GLOBALS['ISC_CLASS_DB']->Quote($GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerId()) . "
						AND r.ordreforderid = o.orderid AND r.ordreferralid = u.refcodeid AND o.deleted = 0 ORDER BY o.orderid DESC";
			$Result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		}
		...
		*/
		...
		{
			$GLOBALS['SNIPPETS']['AccountReferrals'] = "";
			$GLOBALS['AccountReferralItemList'] = "";

			$result = false;
			$this->GetCustomerReferrals($result);
			$customer_details = $this->GetAccountDetails();
			// Load the account details for this user
			if (!empty($customer_details)) {
				$GLOBALS['AccountEmail'] = isc_html_escape($customer_details['custconemail']);
			} else {
				$GLOBALS['AccountEmail'] = '';
			}

			// Are there any orders for this customer
			if ($GLOBALS['ISC_CLASS_DB']->CountResult($result) > 0) {
				while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					$GLOBALS['ReferralDate'] = isc_date(GetConfig('DisplayDateFormat'), $row['orddate']);
					$GLOBALS['ReferralName'] = $row['ordbillfirstname'] . " " . $row['ordbilllastname'];
					switch($row['ordrefstatus']) {
						case 0: {
							$GLOBALS['ReferralStatus'] = "Pending";
							$GLOBALS['ReferralColor'] = "#f5f5f5";
							break;
						}
						case 1: {
							$GLOBALS['ReferralStatus'] = "Approved";
							$GLOBALS['ReferralColor'] = "#ebffe0";
							break;
						}
						case 2: {
							$GLOBALS['ReferralStatus'] = "Void";
							$GLOBALS['ReferralColor'] = "#ffe0e0";
							break;	
						}
					}

					$GLOBALS['SNIPPETS']['AccountReferrals'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("AccountReferralItem");
					$GLOBALS['AccountReferralItemList'] = "";
				}

				$GLOBALS['HideNoReferralsMessage'] = "none";
			}
			else {
				$GLOBALS['HideReferralsList'] = "none";
			}

			$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle(sprintf("%s - %s", GetConfig('StoreName'), GetLang('YourReferrals')));
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("account_referrals");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}
