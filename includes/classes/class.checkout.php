...
class ISC_CHECKOUT
{
	...
	{
		$GLOBALS['AdditionalStylesheets'][] = GetConfig('AppPath').'/templates/Modify/Styles/checkout.css';
	...
	public function GenerateExpressCheckoutConfirmation()
	{
		...
		$confirmation = $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet('ExpressCheckoutConfirmation2');
	...
	*/
	private function ConfirmOrder()
	{
		...
		// ensure that the applied ref code is still valid
		//try {
//			$this->getQuote()->reapplyRefCode(true);
//		}
//		catch(ISC_QUOTE_EXCEPTION $e) {
//			$GLOBALS['CheckoutErrorMsg'] = $e->getMessage();
//		}
	...
	*/
	...
	private function ChooseShippingAddress($errors = array())
	{
		...
		$GLOBALS['ShippingActive'] = 'a';
		$GLOBALS['HideBillInactive'] = '';
		$GLOBALS['HideBillActive'] = 'display:none;';
		$GLOBALS['BillingActive'] = '';
	...
	private function ChooseBillingAddress($errors=array())
	{
		...
		$GLOBALS['ShippingActive'] = '';
		$GLOBALS['HideBillInactive'] = 'display:none;';
		$GLOBALS['HideBillActive'] = '';
		$GLOBALS['BillingActive'] = 'a';
	...
	public function SavePendingOrder()
	{
		...
		// Is it a gift?
		if (isset($_POST['order_is_gift'])) {
			$newOrder['extraInfo']['order_is_gift'] = true;
		}
	...
	public static function getQuoteTotalRows(ISC_QUOTE $quote, $displayIncTax = null, $expandShipping = true)
	{
		...
		// Referral Codes
		$quote->reapplyRefCodes();
		$refcode = $quote->getAppliedRefCodes();
		
		if(!empty($refcode['id'])) {
			$referral = array(
				'type' => 'refcode',
				'label' => getLang('ReferralCode').' ('. $refcode['code'] .')',
				'value' => $refcode['totalDiscount'] * -1,
				'id' => $refcode['id'],
			);
			
			$totalRows['refcode-'.$refcode['id']] = $referral;
		}
...
//		// If a returning customer, hide the referral box
		$entries = getclass('ISC_REFERRALS')->isExistingCustomer();
		if ($entries != false) {
			$quote->removeAllRefCodes();
			$GLOBALS['HideReferrals'] = "none;";
		}
	...
	*/
	...
	{
		return preg_match('#^[A-Z0-9]{3}\-[A-Z0-9]{3}\-[A-Z0-9]{3}\-[A-Z0-9]{3}$#', $code);
	}
	...
	public function BuildOrderConfirmation()
	{
		...
		if (!CustomerIsSignedIn() && !isset($_SESSION['CHECKOUT']['CREATE_ACCOUNT'])) {
			$GLOBALS['HideGiftOption'] = 'display:none;';
		}
		...
		// Now we check if we have an incoming referral code to apply
		if ((isset($_REQUEST['refcode']) && $_REQUEST['refcode'] != '')) {
			$code = trim($_REQUEST['refcode']);

			try {
				$this->getQuote()->applyRefCode($code);

				// Coupon code applied successfully
				$GLOBALS['CheckoutSuccessMsg'] = GetLang('RefCodeAppliedToCart');
			}
			catch(ISC_QUOTE_EXCEPTION $e) {
				$GLOBALS['CheckoutErrorMsg'] = $e->getMessage();
			}
