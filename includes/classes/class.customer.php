	...
	class ISC_CUSTOMER
	{
		...
		*/
		private function CreateAccountStep1($Error = "")
		{
			...
			if(isset($_GET['checking_out'])) {
				$GLOBALS['AdditionalStylesheets'][] = GetConfig('AppPath').'/templates/Modify/Styles/checkout.css';
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("createaccountcheckout");
			} else {
		...
		*/
		...
		/**
		*	Return the first and last name of the current user
		*/
		public function GetCustomerName($customer_id=0)
		{
			...
			if ($customer_id > 0) {
				$query = sprintf("select custconfirstname as first, custconlastname as last from [|PREFIX|]customers where customerid='%d'", $GLOBALS['ISC_CLASS_DB']->Quote($customer_id));
				$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
				if ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					return $row;
				}
		...
		/**
		*	Return the referral code of the current user
		*/
		public function GetCustomerRefCode($customer_id=0)
		{
			if ($customer_id == 0) {
				$customer_id = $this->GetCustomerId();
			}
			...
			if ($customer_id > 0) {
				$query = sprintf("SELECT refcode from [|PREFIX|]refcodes where refcodeuserid='%d'", $GLOBALS['ISC_CLASS_DB']->Quote($customer_id));
				$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
				if ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					return $row['refcode'];
				}
					...
					$this->CreateRefCode();	
					return $this->GetCustomerRefCode();
				}
			}
			return false;
		}
		...
		*/
		public function CreateRefCode($customer_id=0) {
			if ($customer_id == 0) {
				$customer_id = $this->GetCustomerId();
			}
			
			if ($customer_id > 0) {
				$refcode = GenRandAlphaNum();
				$query = sprintf("SELECT refcodeid FROM [|PREFIX|]refcodes WHERE refcode = '%s'", $refcode);
				$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
				if ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					$refcode = $this->CreateRefCode();
					return $refcode;
				}
				else {
					$query = sprintf("INSERT INTO [|PREFIX|]refcodes (refcodeuserid, refcodeamount, refcodeminpurchase, refcode, refreward) VALUES
									('%d', 10, 40, '%s', '10')", $customer_id, $refcode);
					$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
					if($result) {
						return $refcode;	
					}
