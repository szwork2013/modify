...
* Can be used in a loop to iterate over all images in the database and using the photowall image class for each element.
*
* @example
*
* $imageIterator = new ISC_PHOTOWALL_IMAGE_ITERATOR;
* foreach($imageIterator as $imageId => $image) {
* 	// $image is an ISC_PHOTOWALL_IMAGE object
* }
*
* The default SQL selects all images, this can be changed by passing in SQL to the constructor:
*
* $imageIterator = new ISC_PHOTOWALL_IMAGE_ITERATOR('SELECT * from etc...');
*/

class ISC_PHOTOWALL_IMAGE_ITERATOR implements Iterator, Countable {
	/**
	* The database class
	*
	* @var MySQLDb
	...
	*/
	...
	/**
	* The current SQL query that is being used by the class when retrieving photowall images
	*
	* @var string
	*/
	private $sql;
	...
	/**
	* This is the current instance of the ISC_PHOTOWALL_IMAGE class with the current row from the database loaded in
	*
	* @var ISC_PHOTOWALL_IMAGE
	*/
	private $currentRow;
	...
	*
	* @param string $sql (Optional) The SQL query used to retrieve photowall image rows from the database. If not used all rows will be selected from the database
	* @return ISC_PHOTOWALL_IMAGE_ITERATOR
	*/

	public function __construct($sql='')
	{
		if(empty($sql)) {
			$this->sql = ISC_PHOTOWALL_IMAGE::generateGetAllPhotowallImagesFromDatabaseSql();
		} else {
			$this->sql = $sql;
		}
	...
	*/
	...
	/**
	* This function returns the key of the current row. The key is the photowallImageId column from the database.
	* Implemented from as a requirement of the iterator interface.
	*
	* @return integer
	*/
	public function key()
	{
		return $this->currentRow->getPhotowallImageId();
	}

	/**
	* This function loads the next row from the database and populates an instance of the ISC_PHOTOWALL_IMAGE class with the returned table row. This is then set as the current object for the loop.
	* Implemented from as a requirement of the iterator interface.
	*
	* @return void
	* @see currentRow
	...
	*/
	...
	public function next()
	{
		...
		$this->currentRow = new ISC_PHOTOWALL_IMAGE();
