...
class ISC_GIFTCERTIFICATES
{
	...
	public function AddGiftCertificateToCart($certificateData)
	{
		...
		$item
			...
			->setQuantity(1)
			->setSignoff($certificateData['signoff'])
			->setGreeting($certificateData['greeting'])
	...
	public function SendGiftCertificateEmail($giftCertificate)
	{
		...
		$GLOBALS['Greeting'] = isc_html_escape($giftCertificate['giftcertgreeting']);
		$GLOBALS['Signoff'] = isc_html_escape($giftCertificate['giftcertsignoff']);
		...
		foreach($certificates as $item) {
			...
			for($i = 1; $i <= $quantity; ++$i) {
				$newCertificate = array(
					...
					'giftcertgreeting'		=> $item->getGreeting(),
					'giftcertsignoff'		=> $item->getSignoff(),
