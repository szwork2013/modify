...
class ISC_PRODUCT_IMAGE {
	...
	*/
	...
	/**
	* Whether or not this image is selected as the component image for the product it belongs to
	*
	* @var bool
	*/
	protected $_isComponent;
	...
	*/
	...
	/**
	* Returns the image for the given product id that would be used as the base component - if no image is marked as such, the page component will be returned, otherwise the first image according to sort order
	*
	* @param int|string $productId
	* @param bool $hash if true, will treat $productId as a hash string of a product being added, instead of a product id
	* @throws ISC_PRODUCT_IMAGE_DBERROR_EXCEPTION If an unhandled database error occurrs while attempting to fetch product image data
	* @return ISC_PRODUCT_IMAGE or false if no usable image was found
	*/
	public static function getComponentImageForProduct($productId, $hash = false)
	{
		$db = $GLOBALS['ISC_CLASS_DB'];

		if ($hash) {
			$sql = "SELECT /*ISC_PRODUCT_IMAGE::getComponentImageForProduct*/ * FROM `[|PREFIX|]product_images` WHERE imageprodhash = '" . $db->Quote($productId) . "' ORDER BY imageiscomponent desc, imagesort LIMIT 1";
		} else {
			$sql = "SELECT /*ISC_PRODUCT_IMAGE::getComponentImageForProduct*/ * FROM `[|PREFIX|]product_images` WHERE imageprodid = " . (int)$productId . " ORDER BY imageiscomponent desc, imagesort LIMIT 1";
		}
	...
	* @return void
	*/
	public static function deleteModImages($id)
	{
		// select images where imageprodid matches no productid
		$sql = "SELECT * FROM `[|PREFIX|]product_images` WHERE imageprodid = " . $id;
	...
	* @return string
	*/
	public static function generateGetProductImagesFromDatabaseSql($productId, $page = null, $hash = false, $nocomponent = false)
	{
		if ($page !== null) {
		...
		$componentQuery = '';
		if ($nocomponent) {
			$componentQuery = " AND imageiscomponent = 0";
		}

		// nearly all columns are required for populateFromDatabaseRow, so just select *
		if ($hash) {
			$sql = "SELECT /*ISC_PRODUCT_IMAGE::generateGetProductImagesFromDatabaseSql*/ * FROM `[|PREFIX|]product_images` WHERE imageprodhash = '" . $GLOBALS['ISC_CLASS_DB']->Quote($productId) . "'" . $componentQuery . " ORDER BY imagesort";
		} else {
			$sql = "SELECT /*ISC_PRODUCT_IMAGE::generateGetProductImagesFromDatabaseSql*/ * FROM `[|PREFIX|]product_images` WHERE imageprodid = " . (int)$productId .  $componentQuery . " ORDER BY imagesort";
		}
		return $sql;
	}
	...
	*/
	public static function getProductImagesFromDatabase($productId, $page = null, $hash = false, $nocomponent = false)
	{
		$db = $GLOBALS['ISC_CLASS_DB'];
		
		$result = $db->Query(self::generateGetProductImagesFromDatabaseSql($productId, $page, $hash, $nocomponent));
	...
	*/
	public static function copyImagesToProductHash($productId, $productHash)
	{
		...
		foreach ($existingImages as $existingImage) {
			...
			if ($existingImage->getIsComponent()) {
				$save = true;
				$image->setIsComponent(true);
			}
	...
	/**
	* Generates the combinatory product images
	*
	* @param int $productId
	* @return array Array of ISC_PRODUCT_IMAGE instances of the new image copies
	*/
	public static function generateCombinationImages($productId)
	{
		$productId = (int)$productId;
		// First delete old images
		ISC_PRODUCT_IMAGE::deleteModImages($productId);
		...
		$query = "
			SELECT 
				i.*
			FROM 
				[|PREFIX|]product_images i, [|PREFIX|]product_associations a
			WHERE 
				a.assocbaseid = " . $productId . " 
				AND i.imageprodid = a.assocprodid
				AND i.imageiscomponent = '1'
			ORDER BY imagesort
		";
		...
		if(isset($images[0]) && isset($images[1]) && !empty($images[0]) && !empty($images[1])) {
			$imageUrl = ISC_PRODUCT_IMAGE::combineImages($images[0], $images[1]);
		}	
	...
	* @return array Array of ISC_PRODUCT_IMAGE instances of the new image copies
	*/
	public static function combineImages($imageOne, $imageTwo)
	{
		$imageBase = array();
		$imageOverlay = array();
		$newImage = array();
		...
		// First we need to get the image type and create new image of the proper type for each image
		$imageBase['ext'] = ISC_PRODUCT_IMAGE::getImageType($imageOne);
		ISC_PRODUCT_IMAGE::createGDImage($imageBase['ext'], $imageBase['instance'], $imageOne);

		$imageOverlay['ext'] = ISC_PRODUCT_IMAGE::getImageType($imageTwo);
		ISC_PRODUCT_IMAGE::createGDImage($imageOverlay['ext'], $imageOverlay['instance'], $imageTwo);

		// Size 'em up!
		ISC_PRODUCT_IMAGE::getImageDimensions($imageBase['instance'], $imageBase['width'], $imageBase['height']);
		ISC_PRODUCT_IMAGE::getImageDimensions($imageOverlay['instance'], $imageOverlay['width'], $imageOverlay['height']);
		ISC_PRODUCT_IMAGE::getSmallDimensions($newImage['width'], $newImage['height'], $imageBase['width'], $imageBase['height'], $imageOverlay['width'], $imageOverlay['height']);
		
		// Make sure the images are the same size
		if($imageBase['width'] !== $imageOverlay['width']) {
			if($imageBase['width'] < $imageOverlay['width']) {
			...
			if($imageBase['width'] < $imageOverlay['width']) {
				imagecopyresampled($new, $imageOverlay['instance'], 0, 0, $x, 0, $width, $height, $imageOverlay['width'], $imageOverlay['height']);
				$imageOverlay['instance'] = $new;
				unset($new);
			} else {
				imagecopyresampled($new, $imageBase['instance'], 0, 0, $x, 0, $width, $height, $imageBase['width'], $imageBase['height']);
				$imageBase['instance'] = $new;
				unset($new);
			}
		}

		// Create the new image canvas
		$newImage['instance'] =  @imagecreatetruecolor($newImage['width'], $newImage['height']);
		$grey = imagecolorallocate($newImage['instance'], 192, 192, 192);
		imagefill($newImage['instance'], 0, 0, $grey);

		imagecopy($newImage['instance'], $imageOverlay['instance'], 0, 0, 0, 0, $imageOverlay['width'], $imageOverlay['height']);
		imagecopy($newImage['instance'], $imageBase['instance'], 0, 0, 0, 0, $imageBase['width'], $imageBase['height']);
		
		$newFileName = ISC_PRODUCT_IMAGE::generateSourceImageRelativeFilePath(time().".jpg");
		$newImage['url'] = ISC_BASE_PATH . '/' . GetConfig('ImageDirectory') . '/' . $newFileName;
		
		if(!is_dir(substr($newImage['url'], 0, -21))) {
			isc_mkdir(substr($newImage['url'], 0, -21));
		}
		
		if(!imagejpeg($newImage['instance'], $newImage['url'])) {
	...
	public static function copyMergeAlpha($dst_im, $src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, $pct, $trans = NULL)
	{
	...
	public static function getSmallDimensions(&$newWidth, &$newHeight, $imageOneW, $imageOneH, $imageTwoW, $imageTwoH) 
	{
		if(($imageOneW * $imageOneH) < ($imageTwoW * $imageTwoH)) {
			$newWidth = $imageOneW;
			$newHeight = $imageOneH;
		} else {
			$newWidth = $imageTwoW;
	...
	public static function getImageDimensions($image, &$width, &$height) {
		$width = imagesx($image);
		$height = imagesy($image);
		return;
	}
	
	public static function createGDImage($extension, &$image, $url) {
		$dir = ISC_BASE_PATH . '/' . GetConfig('ImageDirectory') . '/';
		switch($extension) {
			case 'jpg':
				$image = @imagecreatefromjpeg($dir.$url);
				break;
			case 'jpeg':
				$image = @imagecreatefromjpeg($dir.$url);
				break;
			case 'png':
				$image = @imagecreatefrompng($dir.$url);
				break;
			case 'gif':
				$image = @imagecreatefromgif($dir.$url);
				break;
		}
	...
	/**
	* Get The Image Type Based on URl
	*
	* @param url $url
	* @return array Array of ISC_PRODUCT_IMAGE instances of the new image copies
	*/
	...
	{
		$extension = substr($url, strpos($url, '.')+1);
		if(strpos($extension, '.')) {
			$extension = substr($url, strpos($url, '.')+1, 1);
		}
	...
	*/
	public function populateFromDatabaseRow($row)
	{
		...
		$this->setIsComponent($row['imageiscomponent'] == 1);
	...
	*/
	public function saveToDatabase($generateImages = true)
	{
		...
		if ($this->getIsComponent()) {
			$data['imageiscomponent'] = '1';
		}
		...
		if (!$productImageId) {
			...
			if ($this->getIsComponent()) {
				// this image is the thumbnail, make sure others are not
				if ($this->getProductHash()) {
					$db->Query("UPDATE /*ISC_PRODUCT_IMAGE->saveToDatabase*/ `[|PREFIX|]product_images` SET imageiscomponent = 0 WHERE imageid <> " . $this->getProductImageId() . " AND imageprodhash = '" . $db->Quote($this->getProductHash()) . "'");
				} else if ($this->getProductId()) {
					$db->Query("UPDATE /*ISC_PRODUCT_IMAGE->saveToDatabase*/ `[|PREFIX|]product_images` SET imageiscomponent = 0 WHERE imageid <> " . $this->getProductImageId() . " AND imageprodid = " . $this->getProductId());
				}
			}
	...
	*/
	...
	/**
	* Flags the current image as the preferred component for the product it's related to. Does not automatically update other images
	*
	...
	*/
	...
	/**
	* Returns whether the current image is the preferred component for this product or not
	*
	* @return bool True if the image is the preferred component.
	*/
	...
	* @return void
	*/
	public function delete($loadFirst = true, $deleteFiles = true, &$newThumbnailId = null, $adjustSort = true, &$newComponentId = null)
	{
		$newThumbnailId = null;
		$newComponentId = null;
		...
		// if necessary, set another image as the component
		if ($this->getIsComponent()) {
			// find the next thumbnail candidate
			if ($this->getProductId()) {
				$sql = "SELECT /*ISC_PRODUCT_IMAGE->delete*/ imageid FROM `[|PREFIX|]product_images` WHERE imageprodid = " . $this->getProductId() . " ORDER BY imagesort ASC LIMIT 1";
			...
			$row = $db->Fetch($result);
			if ($row !== false) {
				// update it to be the new component image
				if (!$db->Query("UPDATE /*ISC_PRODUCT_IMAGE->delete*/ `[|PREFIX|]product_images` SET imageiscomponent = 1 WHERE imageid = " . (int)$row['imageid'])) {
					$db->RollbackTransaction();
					throw new ISC_PRODUCT_IMAGE_DBERROR_EXCEPTION(sprintf(GetLang('ProductImageDatabaseError'), __CLASS__, __METHOD__, $db->GetErrorMsg()));
				}

				$newComponentId = (int)$row['imageid'];
			}
		}
