<?php
	...
	class ISC_PRODUCT
	{
		...
		public $_prodquoteauthor = false;

		public $_prodname = "";
		public $_prodthumb = "";
		public $_proddesc = "";
		public $_prodfunnydesc = "";
		...
		public function __construct($productid=0)
		{
			...
			$GLOBALS['AdditionalStylesheets'][] = GetConfig('AppPath').'/templates/Modify/Styles/prodpage.css';
		...
		public function _SetProductData($productid=0)
		{
			...
			$this->_prodnamedby = $row['prodquoteauthor'];
			$this->_prodphystype = $row['prodphystype'];
			$this->_prodsku = $row['prodcode'];
			$this->_prodthumb = $row['imagefile'];
			$this->_proddesc = $row['proddesc'];
			$this->_prodfunnydesc = $row['prodfundesc'];
		...
		*/
		...
		public function GetProductNamedBy()
		{
			if(!$this->_prodnamedby) {
				return "The Modify Team";
			}
			
			return $this->_prodnamedby;
		}
		...
		public function GetFunnyDesc()
		{
			return $this->_prodfunnydesc;
		}
		...
		*/
		...
		public function ShowQuickView()
		{
			if ($this->_prodid > 0) {
				...
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle($this->BuildTitle());
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("fastproduct");
				$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
			}
			// Visiting an invalid product, show a lovely error message
			else {
