...
class ISC_QUOTE
{
	...
	/**
	 * @var array Associative array containing referral codes
	 */
	protected $refcode = array();
	...
	*/
	...
	public function addRefCode($coupon)
	{
		$this->refcode = $coupon;
	}
	 ...
	 *
	 * @param boolean $throwException Determine if this function w	ould throw exception, defaulted to false
	 * @return ISC_QUOTE This quote instance.
	 */
	...
	/**
	 * Re-apply all previously applied referral codes on this quote. To reflect
	 * the new discounts offered by items just added, or remove referral
	 * codes that no longer apply due to quote changes.
	 *
	 * @param boolean $throwException Determine if this function would throw exception, defaulted to false
	 * @return ISC_QUOTE This quote instance.
	 */
	public function reapplyRefCodes($throwException = false)
	{
		$refcode = $this->refcode;
		$this->removeAllRefCodes();
		
		// Now go ahead and re-apply
		try {
			if((empty($refcode['id']) ||  !isset($refcode)) && isset($_COOKIE['REFERRING_CODE']) && !empty($_COOKIE['REFERRING_CODE'])) {
				$this->applyRefCode($_COOKIE['REFERRING_CODE']);
			}
			else if(!empty($refcode)) {
				$this->applyRefCode($refcode['code']);
			}
		}
		catch(ISC_QUOTE_EXCEPTION $e) {
			if ($throwException) {
	...
	*/
	...
	public function applyRefCode($refcode)
	{
		$this->removeAllRefCodes();

		if(is_string($refcode) || is_numeric($refcode)) {
			// Look up the ref code
			$code = trim($refcode);
			$refcode = $this->fetchRefCode($code);

			if (!$refcode || !$refcode['refcode']) {
				throw new ISC_QUOTE_EXCEPTION(GetLang('InvalidRefCode'), ISC_QUOTE_EXCEPTION::REFCODE_INVALID);
			}
		}

		// check if the customer is a new customer
		$entries = getclass('ISC_REFERRALS')->isExistingCustomer();
		if ($entries != false) {
			throw new ISC_QUOTE_EXCEPTION(getLang('InvalidRefCodeNotNew'), ISC_QUOTE_EXCEPTION::REFCODE_NOT_NEW);
			return false;
		}

		// Does the cart subtotal meet the minimum purchase amount required?
		$subtotal = $this->getBaseSubTotal();
//		if (isset($refcode['refcodeminpurchase']) && $refcode['refcodeminpurchase'] > 0 && $subtotal < $refcode['refcodeminpurchase']) {
//			$amountDiff = $refcode['refcodeminpurchase'] - $subtotal;
//			$amountDiff = CurrencyConvertFormatPrice($amountDiff);
//			$invalidRefMesg = sprintf(GetLang('InvalidRefCodeMinPrice'), $amountDiff);
//			throw new ISC_QUOTE_EXCEPTION($invalidRefMesg, ISC_QUOTE_EXCEPTION::REFCODE_MIN_PURCHASE);
//		}
		...
		foreach ($this->items as $item) {
			...
			if(!empty($this->coupons)) {
				$discountAmount = $item->getDiscountedBaseTotal() * .01;
				$discountAmount = round($discountAmount, getConfig('DecimalPlaces'));
			}
			else {
				$discountAmount = $item->getDiscountedBaseTotal() * ($refcode['refcodeamount'] / 100);
				$discountAmount = round($discountAmount, getConfig('DecimalPlaces'));
			}
				
			// Add the refcode in under 'refcode'
			$totalRefDiscount += $discountAmount;
			$item->addDiscount('refcode', $discountAmount);
		}

		if ($totalRefDiscount > $subtotal) {
			$totalRefDiscount = $subtotal;
		}

		if(!$appliesToQuote) {
				throw new ISC_QUOTE_EXCEPTION(GetLang('InvalidRefCode'), ISC_QUOTE_EXCEPTION::REFCODE_DOES_NOT_APPLY);
		}
		
		$code = array(
			'id' => $refcode['refcodeid'],
			'code' => $refcode['refcode'],
			'owner' => $refcode['refcodeuserid'],
			'discountAmount' => $refcode['refcodeamount'],
			'totalDiscount' => round($totalRefDiscount, getConfig('DecimalPlaces')),
		);

		$this->addRefCode($code);
		return $this;
	}
	
	public function removeAllRefCodes()
	{
		$this->removeRefCode();
	}
	...
	public function fetchRefCode($code)
	{
		$query = "
			SELECT *
			FROM [|PREFIX|]refcodes
			WHERE refcode='".$GLOBALS['ISC_CLASS_DB']->quote($code)."'
		";
		$result = $GLOBALS['ISC_CLASS_DB']->query($query);
		$coupon = $GLOBALS['ISC_CLASS_DB']->fetch($result);
	...
	public function removeRefCode()
	{
		// Remove the coupon from the item
		foreach ($this->items as $item) {
			$item->addDiscount('refcode', 0);
			$item->addDiscount('total-refcode', 0);
		}
	 ...
	 * @return array Array of coupon codes applied to the quote.
	 */
	public function getAppliedRefCodes()
	{
		return $this->refcode;
	}
	 ...
	 * referral codes.
	 *
	 * @return float Total discount off the quote total from referral codes.
	 */
	public function getReferralDiscount()
	{
		$discount = 0;
		$refcode = $this->refcode;
		if(isset($refcode['totalDiscount'])) {
			$discount = $refcode['totalDiscount'];
		}
		else {
			$discount = 0;	
		}
