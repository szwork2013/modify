...
class ISC_CARTCONTENT_PANEL extends PANEL
{
	...
	public function generateCartContent()
	{
		...
		$GLOBALS['TotalNumCartItems'] = 0;
		foreach($items as $item) {
			$name = $item->getName();
			$quantity = $item->getQuantity();
			
			$GLOBALS['TotalNumCartItems'] += $quantity;
			...
			$GLOBALS['ProductType'] = $item->getType();
			
			$subscription = false;
			if($GLOBALS['ProductType'] == 5) {
				$subscription = true;
				$GLOBALS['ShowSubscription'] = '';
			}
			else {
				$GLOBALS['ShowSubscription'] = 'display:none;';	
			}
		...
		if($subscription) {
			flashMessage(getLang('SubscriptionInCart'), MSG_INFO);	
		}
