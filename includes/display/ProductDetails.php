...
CLASS ISC_PRODUCTDETAILS_PANEL extends PANEL
{
	...
	public function SetPanelSettings()
	{
		...
		$GLOBALS['ProductLink'] = ProdLink($this->productClass->GetProductName());

		$GLOBALS['DisplayStrapMat'] = "";
		$GLOBALS['DisplayFaceMat'] = "";
		switch($this->productClass->GetPhysType()) {
			case 1:
				$GLOBALS['DisplayStrapMat'] = "none;";
				break;
			case 2:
				$GLOBALS['DisplayFaceMat'] = "none;";
				break;
		}
	...
	private function SetProductVariations()
	{
		...
		foreach($variationOptions as $optionName) {
			...
			if($onlyOneVariation && count($variationValues[$optionName]) <= 5 && !$this->productClass->IsOptionRequired()) {
				...
				$GLOBALS['OptionSelected'] = "class='a'";
